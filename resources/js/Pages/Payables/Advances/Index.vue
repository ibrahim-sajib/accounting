...
const openCreate = () => {
    form.clearErrors();
    form.reset();
    form.payment_date = props.today ?? new Date().toISOString().slice(0, 10);
    form.account_id = props.accounts[0]?.value ?? ('' as number | '');
    showCreate.value = true;
};

const submit = () => {
    form.post(route('supplier-advances.store'), {
        preserveScroll: true,
        onSuccess: () => {
    ...
    <AuthenticatedLayout>
                ...
                <table class="min-w-[760px] w-full divide-y divide-gray-200 text-sm dark:divide-gray-800">
                    ...
                    <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
                        <tr v-for="row in advances.data" :key="row.id" class="hover:bg-gray-50 dark:hover:bg-gray-800/50">
                            <td class="px-4 py-3">
                                <Link :href="route('supplier-advances.show', row.id)" class="font-mono font-medium text-indigo-600 hover:text-indigo-800 dark:text-indigo-400">
                                    {{ row.payment_no ?? 'Advance' }}
                                </Link>
                            </td>
                            <td class="px-4 py-3 font-medium text-gray-900 dark:text-white">{{ row.supplier?.name ?? '—' }}</td>
                            <td class="px-4 py-3 text-right font-mono text-gray-700 dark:text-gray-200">{{ formatDate(row.payment_date) }}</td>
        ...
        <Modal :show="showCreate" max-width="md" @close="showCreate = false">
            <form class="p-6" @submit.prevent="submit">
                <h2 class="text-lg font-semibold text-gray-900 dark:text-white">Record Supplier Advance</h2>
                <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                    Journal: Supplier Advances (Asset) Dr | Cash/Bank Cr. The advance can be applied to future bills.
                </p>

                <div class="mt-5 grid grid-cols-1 gap-4 sm:grid-cols-2">
                    <div class="sm:col-span-2">
                        <InputLabel for="adv_supplier" value="Supplier" />
                        <select
                            id="adv_supplier"
                    ...
                    <div>
                        ...
                        <TextInput id="adv_date" v-model="form.payment_date" type="date" class="mt-1 block w-full" required />
                        <InputError class="mt-2" :message="form.errors.payment_date" />
                    </div>

                    <div>
                        <InputLabel for="adv_account" value="Pay from account" />
                        <select
                            id="adv_account"
