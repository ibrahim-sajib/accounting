    ...
    <AuthenticatedLayout>
        <div>
            ...
            <div class="mb-4 grid grid-cols-1 gap-3 sm:grid-cols-3">
                ...
                <div class="rounded-xl border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-800 dark:bg-gray-900">
                    <div class="text-xs uppercase tracking-wide text-gray-400">Open invoices</div>
            ...
            <div class="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center">
                <div class="w-full sm:max-w-xs">
                    <SearchInput :model-value="filters?.search" placeholder="Search invoice no. or customer..." query-param="search" />
                </div>
                <select
            ...
            <div v-else class="overflow-x-auto rounded-xl border border-gray-200 bg-white shadow-sm dark:border-gray-800 dark:bg-gray-900">
                <table class="min-w-[760px] w-full divide-y divide-gray-200 text-sm dark:divide-gray-800">
                    ...
                    <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
                        ...
                        >
                            <td class="px-4 py-3">
                                <Link :href="route('sales.invoices.show', row.id)" class="font-mono font-medium text-indigo-600 hover:text-indigo-800 dark:text-indigo-400">
                                    {{ row.invoice_no ?? 'Draft' }}
                                </Link>
                                <div class="text-xs text-gray-400">{{ row.invoice_date ? formatDate(row.invoice_date) : '—' }}</div>
                            </td>
